...
import type { SeriesItem, SeriesStatus } from '@/db/schema';

interface SeriesEditorModalProps {
  existing?: SeriesItem;
  onClose: () => void;
  onSave: (input: Omit<SeriesItem, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt' | 'type'>) => void;
}

const STATUS_OPTIONS: { value: SeriesStatus; label: string }[] = [
  { value: 'wantTo', label: 'Want to Watch' },
  { value: 'watching', label: 'Watching' },
  { value: 'finished', label: 'Finished' },
  { value: 'didNotFinish', label: 'Did Not Finish' },
];

export default function SeriesEditorModal({ existing, onClose, onSave }: SeriesEditorModalProps) {
  const [title, setTitle] = useState(existing?.title ?? '');
  const [posterImageId, setPosterImageId] = useState(existing?.posterImageId);
  const [status, setStatus] = useState<SeriesStatus | undefined>(existing?.status);
  ...
  return (
    <Modal onClose={onClose}>
      <ModalHeader icon="📺" title={existing ? 'Edit Series' : 'Add a Series'} onClose={onClose} />
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-2">
        ...
        >
          {existing ? 'Save Changes' : 'Add Series'}
        </button>
